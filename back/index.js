...
const con = mysql.createConnection({ host: config.mysql.host, user: config.mysql.user, password: config.mysql.pass, database: config.mysql.name });

const sequelize = new Sequelize({ host: config.mysql.host, username: config.mysql.user, password: config.mysql.pass, database: config.mysql.name, dialect: "mysql" });
...
// requête sql pour avoir les email des users
app.get("/user/info_email", function (req, res) {
  con.query("SELECT `email` FROM `user`",
...
app.post("/upload/image/:id/:table/:param", (uploadImageFile.single("images")),  (req, res) => {
    const id = req.params.id
    const table = req.params.table
    const param = req.params.param
    const img = req.file.filename;
    const sql = "UPDATE " + table + " set " + param + " = ? Where id = " + id;
    con.query(sql, [img], (err, result) => {
